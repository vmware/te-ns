...
class AviTE():
		...
		Args:
            te_controller: dict, required
                The Controller Credientials which includes
                    ...
                    user      : Must be a root user -- needs to run docker container
    ...
    def __checkIfContainerIsUp(self, DICT_OF_SERVICES, maxTolerableDelay, maxRetry, ssh):
        ...
        for i in range(maxRetry):
            ...
            for service, port in DICT_OF_SERVICES.items():
                cmd = "netstat -planet | grep %s | wc -l" %(port)
                (type, listOut) = self.__exec_cmd_remote(cmd, ssh)
                if(type == self.__ERR):
    ...
    def __setup_te_from_docker_file(self, pathToTrafficEngine, stat_collect_interval, \
        ...
        pwd = self.__te_controller.get('passwd', 'avi123')
        ...
        def __update_free_port():
            ...
            if(type == self.__ERR or not(bool(listOut))):
                return False, "Please install netstat in remote TE Controller machine"
            cmd = "netstat -ltpn | awk '{print $4}' | grep '[0-9]' | cut -d ':' -f2  | uniq"
            (type, listOut) = self.__exec_cmd_remote(cmd, ssh)
            if type == self.__ERR:
        ...
        def __buildDocker(type, image_id=None):
            ...
            oldImageID = __getCurrentImageID(type)
            cmd = ""
            if type == "TE_DP":
            ...
            newImageID = __getCurrentImageID(type)
        ...
        if(listOut == []):
            ...
            scp.put(os.path.join(pathToTrafficEngine, "te", buildImages["TE"]["docker_file_name"]), '/root/')
            ...
            cmd='docker load -i %s' %os.path.join('/root/' ,buildImages["TE"]["docker_file_name"])
            (type, listOut) = self.__exec_cmd_remote(cmd, ssh)
    ...
    def __setup_te_from_repo(self, repo_details, stat_collect_interval, stat_dump_interval, \
        ...
        scp.put(fileToSCP, '/root/GET_AND_RUN_DOCKER_IMAGE.py')
        cmd = "python /root/GET_AND_RUN_DOCKER_IMAGE.py -w /root/ -ip %s -p %s -b %s -t TE -h_ip %s \
            ...
            te_dp_dict: dict, required
                The TE_DP client credientials which includes:
                    host_ip : Keys to te_dp_dict (Client mgmt IP) whose values includes:
                        user : Root user to the host_ip
                        passwd : Root user's password to the host_ip
        Example:
            te_dp_dict = {
                '1.1.1.1' : {'passwd': 'avi123', 'user': 'root'},
                '2.2.2.2' : {'passwd': u'avi123', 'user': u'root'}
            }
    ...
    @__retry_wrapper
    def tech_support(self, te_dp_dict={}, scp_ip=None, scp_user='root', scp_passwd='avi123', \
        ...
        if url is not None:
            if scp_ip is None:
                ...
                scp_passwd = self.__te_controller.get('passwd', 'avi123')
