        ...
        try:
            ...
            for i in range(maxRetries+1):
                ...
                #Task Failed due to RQ Error
                    ...
                    taskResult["status"] = False
                    self.lgr.debug("RQ-Failure in host_ip=%s" %(self.__credentials['host_ip']))
                    self.__task_mapping[typeOfTask].pop(cpu)
            ...
            for i in range(maxRetries+1):
                ...
                for cpu in list_of_cpus:
                    ...
                    #Task Failed due to RQ Error
                        ...
                        taskResult["status"] = False
                        self.lgr.debug("RQ-Failure for task_profile_tag=%s in host_ip=%s" %(task_profile_tag, self.__credentials['host_ip']))
                        self.__task_mapping[typeOfTask].pop(cpu)
