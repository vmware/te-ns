...
class TE_POSTGRES:
    ...
    def __get_latency_percentile(self, metric_json):
        ...
        if bool(mean_lat) and bool(sd_lat):
            ...
            metric_json['latency_mean'] = str(round(mean_lat, 5))
            metric_json['latency_sd'] = str(round(sd_lat, 5))
            metric_json['latency_p10'] = str(round(np.percentile(values, 10), 5))
            metric_json['latency_p90'] = str(round(np.percentile(values, 90), 5))
            metric_json['latency_p95'] = str(round(np.percentile(values, 95), 5))
            metric_json['latency_p99'] = str(round(np.percentile(values, 99), 5))
            metric_json['latency_p99.9'] = str(round(np.percentile(values, 99.9), 5))
            ...
            for key in keys_list:
                ...
                if(dict_key not in current):
                    ...
                    if(counter!=num_levels):
                        ...
                        if(is_error_metrics):
                            current[dict_key] = [{'error_count' : str(data['sum']),
                ...
                #ERROR Metrics can have multiple entries for same key at the last level
                    ...
                    current[dict_key].append({'error_count' : str(data['sum']),
                                'time-stamp-range' : [data['start_date'],
                                                    data['end_date']]})
