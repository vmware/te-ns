...
class DOCKER_INITIALIZER:
    ...
    def __init__(self, pathToDocker, ip, nginxPort, stat_collect_interval, stat_dump_interval, \
        ...
        docker_type_map = {
            ...
            'TE_DP' : {
                ...
                'run_cmd'             : "docker run --privileged --cap-add=SYS_PTRACE --security-opt " \
                                        "seccomp=unconfined -v /tmp/:/te_host/ -v /root/:/te_root/ " \
        ...
        cmd =  "rm -f /root/download_docker.log && touch /root/download_docker.log"
        self.__exec_cmd(cmd)
        self.lgr = Logger("init","/root/download_docker.log").getLogger()
    ...
    def __update_run_cmd(self):
        try:
            ...
            (out, err) = self.__exec_cmd("netstat -ltpn | awk '{print $4}' | grep '[0-9]' | cut -d ':' -f2  | uniq")
            if(bool(err)):
